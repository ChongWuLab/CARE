...
#CARE2: based on the measurement error idea (no overlapped samples. This version is for real data pipeline)
...
CARE2_boot_overlap <- function(gamma.exp_sel, gamma.out_sel, se.exp_sel, se.out_sel, nx,ny, overlap.mat, nrep = 1000, algorithm = "Lasso",random_start = 0,biascorrect = "no",etamean = 0.5, pthr = 5e-5)
{
    ...
    if(algorithm == "Lasso") {
        ...
        MRcML_result = cMLCOverlap_CD_boot_efron(b_exp = gamma.exp_sel, b_out = gamma.out_sel, se2_exp = se2.exp_sel, se2_out = se2.out_sel, K_vec = KvecList,wAll = wAll, random_start = random_start, maxit = 1000,  n = min(nx,ny), nrep = nrep, se_exp = se.exp_sel, se_out = se.out_sel, rho = rho) # coordinate descent type algorithm; based on Haoran & Pan AJHG paper; this is problematic
    ...
    #sqrt(sum(wAll3^2))
    #MA_se_efron2 = sqrt(sum( (wAll3 + wAllD / sqrt(dim(wAll)[1]) )^2 ) )
    #MA_se_efron3 = sqrt(sum( (wAll3 + wAllD / (dim(wAll)[1])^1/4 )^2 ) )
    ...
    #sqrt(sum(wAll3^2))
    #BIC_se_efron2 = sqrt(sum( (wAll3 + wAllD / sqrt(dim(wAll)[1]) )^2 ) )
    #BIC_se_efron3 = sqrt(sum( (wAll3 + wAllD / (dim(wAll)[1])^1/4 )^2 ) )
