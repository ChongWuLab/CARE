...
{ // This function contains bugs, we use the R version first to see if the idea are correct...
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
double mr_L2_measureC(arma::vec b_exp, arma::vec b_out, arma::vec se2_exp, arma::vec se2_out, arma::vec w, arma::vec r)
{

    double a = arma::accu(w % ((arma::pow(b_exp, 2) - se2_exp % (1 - arma::abs(r) ) ) / se2_out));
    double b = arma::accu(w % ( (b_exp % (b_out - r) ) / se2_out ) );
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
Rcpp::List cML_L2_estimateC(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec w, const double gamma, double initial_theta, const int maxit)
{
    ...
    double dlx = 1.0;
    // For simplicity, we use K to represent gamma to reuse codes easily.
    // This is the key parts to be revised
    while ((dlx > 1e-5) & (iteindx < maxit))
    {
        thetaold = theta;
        iteindx = iteindx + 1;
        // Update r_j with weighted loss and unweighted penalty
        for (int j = 0; j < p; j++) {
            double gammaused = gamma + 0.5 * theta * theta * se2_exp(j) / se2_out(j);
            double diff = b_out(j) - theta * b_exp(j);
            double threshold = gammaused * se2_out(j);
            if (std::abs(diff) > threshold) {
                vbg(j) = (diff > 0 ? 1 : -1) * (std::abs(diff) - gammaused * se2_out(j));
            } else {
                vbg(j) = 0;
            }
        }

        arma::uvec tmpindx2 = arma::find(vbg == 0);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
Rcpp::List cML_L2_randomC(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec &w, const double gamma, const int random_start, const int maxit, double initial_theta)
{
    ...
    for (int j = 0; j <= random_start; j++)
    {
        ...
        Rcpp::List MLEresult = cML_L2_estimateC(b_exp, b_out, se2_exp, se2_out, w, gamma, initial_theta, maxit);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
arma::vec mr_CARE_L2(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec K_vec, arma::vec w, const int random_start, const int maxit, const int n)
{
    ...
    for (int j = 0; j < M; j++)
    {
        double gamma = K_vec(j);
        Rcpp::List randRes = cML_L2_randomC(b_exp, b_out, se2_exp, se2_out, w, gamma, random_start, maxit, initial_theta);
        ...
        arma::vec rvec = randRes[2];
        bicdf(j) = arma::accu(rvec != 0);

        // arma::vec tmpInvalid = randRes[3];
        // invalid_mat.col(j) = tmpInvalid;
    }

    arma::vec BIC_v = log(n) * bicdf + 2 * l_v;
    BIC_v = BIC_v - BIC_v.min();
    arma::vec weight_v = arma::exp(-0.5 * BIC_v);
    weight_v = weight_v / arma::accu(weight_v);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
arma::mat mr_CARE_L2_boot_efron(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, Rcpp::List K_vec, arma::mat wAll, const int random_start, const int maxit, const int n, const int nrep)
{
    ...
    for (int j = 0; j < nrep; j++)
    {
        ...
        arma::vec tmp = mr_CARE_L2(b_exp2, b_out2, se2_exp2, se2_out2, K_vec2, w2, random_start, maxit, n);
        // Rcout << "Finish test" << j << std::endl;
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
Rcpp::List cML_L1_estimateC(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec w, const double gamma, double initial_theta, const int maxit)
{
    ...
    while ((dlx > 1e-5) & (iteindx < maxit))
    {
        ...
        for (int j = 0; j < p; j++) {
            double diff = b_out(j) - theta * b_exp(j);
            double threshold = (gamma * se2_out(j)) + std::abs(theta) * std::sqrt(se2_exp(j));
            if (std::abs(diff) > threshold) {
                vbg(j) = (diff > 0 ? 1 : -1) * (std::abs(diff) - gamma * se2_out(j) );
            } else {
                vbg(j) = 0;
            }
        }
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
Rcpp::List cML_L1_randomC(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec &w, const double gamma, const int random_start, const int maxit, double initial_theta)
{
    ...
    for (int j = 0; j <= random_start; j++)
    {
        ...
        Rcpp::List MLEresult = cML_L1_estimateC(b_exp, b_out, se2_exp, se2_out, w, gamma, initial_theta, maxit);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
arma::vec mr_CARE_L1(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec K_vec, arma::vec w, const int random_start, const int maxit, const int n)
{
    ...
    {
        double gamma = K_vec(j);
        Rcpp::List randRes = cML_L1_randomC(b_exp, b_out, se2_exp, se2_out, w, gamma, random_start, maxit, initial_theta);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
arma::mat mr_CARE_L1_boot_efron(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, Rcpp::List K_vec, arma::mat wAll, const int random_start, const int maxit, const int n, const int nrep)
{
    ...
    for (int j = 0; j < nrep; j++)
    {
        ...
        arma::vec tmp = mr_CARE_L1(b_exp2, b_out2, se2_exp2, se2_out2, K_vec2, w2, random_start, maxit, n);
        // Rcout << "Finish test" << j << std::endl;
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
Rcpp::List mr_L1_CD3(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec K_vec, arma::vec w, const int random_start, const int maxit, const int n)
{ // output the invalid IVs
    ...
    {
        int K = K_vec(j);
        Rcpp::List randRes = cML_L1_randomC(b_exp, b_out, se2_exp, se2_out, w, K, random_start, maxit, initial_theta);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
Rcpp::List mr_L2_CD3(arma::vec &b_exp, arma::vec &b_out, arma::vec &se2_exp, arma::vec &se2_out, arma::vec K_vec, arma::vec w, const int random_start, const int maxit, const int n)
{ // output the invalid IVs
    ...
    {
        int K = K_vec(j);
        Rcpp::List randRes = cML_L2_randomC(b_exp, b_out, se2_exp, se2_out, w, K, random_start, maxit, initial_theta);
