...
#create output:
save_datdir = getwd()
save_datdir = paste(save_datdir,"/simRes6/",sep="")
...
for(sim.ind in simulation.ind.set)
{
    ...
    while(numIV <3 | numIV2 < 4) {
        ...
        # Sample 100 unique pairs of markers
        sampled_indices <- t(combn(n_snps, 2, simplify=TRUE))
        sampled_pairs <- sampled_indices[sample(nrow(sampled_indices), 100), ]

        # Compute interaction terms
        interaction_terms <- matrix(nrow = nrow(G), ncol = 100)
        for (i in seq_len(100)) {
            interaction_terms[,i] <- G[, sampled_pairs[i,1]] * G[, sampled_pairs[i,2]]
        }
        X <- G %*% gamma[causalsnps] + (G^2) %*% gamma[causalsnps] + thetaUx * U + rowSums(interaction_terms) + Ex 

        # 3. generate outcome Y
        Y <- theta * X + thetaU * U + G %*% alpha[causalsnps] + Ey
