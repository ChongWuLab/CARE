...
#create output:
save_datdir = getwd()
save_datdir = paste(save_datdir,"/simRes17/",sep="")
...
for(sim.ind in simulation.ind.set) {
  ...
  while(numIV <3 | numIV2 < 4) {
      ...
      # Valid IVs: U(0.03, 0.1)
      gamma[ind1] = runif(length(ind1), min = 0.001, max = 0.006)
      
      # Invalid IVs: U(0.03, 0.1) for gamma, U(-0.1, 0.1) for alpha
      gamma[ind2] = runif(length(ind2), min = 0.001, max = 0.006)
      
      ind2all = ind2
      
      ind2 = ind2all
      alpha[ind2] = runif(length(ind2), min = -0.01, max = 0.01)
      
      # IVs that fail the relevance assumption
